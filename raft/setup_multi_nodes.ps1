...
docker network create raft-network

# Step 3: Create 10 Docker Containers but don't start them

$totalNodes = 5

for ($numNodes = 5; $numNodes -le $totalNodes; $numNodes+=5) {

    Write-Host "Creating $numNodes nodes..."

    for ($j = 1; $j -le $numNodes; $j++) {
        $port = 5000 + $j
        docker create --name "node$j" --network raft-network -p "${port}:${port}" raft-node $j "$port" Nodes_map.json $numNodes
    }
...
docker network rm raft-network
